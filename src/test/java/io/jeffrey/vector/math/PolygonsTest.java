...
public class PolygonsTest {
    ...
    public void testInside() {
        ...
        VectorRegister6 temp = new VectorRegister6();
        for (double x = -0.5; x <= 5; x += 0.1) {
            for (double y = -0.5; y <= 5; y += 0.1) {
                for (int k = 0; k < 1000; k++) {
                    ...
                    reg.set_1(2, 2);
                    reg.set_2(3, 1);
                    double[] coordinates = new double[] { 1, 1, 2, 2, 3, 1 };
                    reg.set_3(x, y);
                    boolean expected = Triangles.pointInsideTriangle_Destructively(reg);
                    boolean actual = Polygons.pointIn(x, y, coordinates, temp);
